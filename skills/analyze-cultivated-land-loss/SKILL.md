...
- 增量包面积是所有具有可计算几何的原始地块面积之和，不按地类编码或其他属性过滤；空编码和未知编码仍计入增量包面积，只从后续分类指标中跳过。
...
- 非农化、非粮化及非粮化两个分项均只在耕地不合理流出范围内按增量包新地类统计；各自占永农面积为对应结果与永久基本农田的相交面积。
- 新增耕地为“上年度非耕地”与“增量包中 `0101`、`0102`、`0103` 耕地”分别融合后求交集所得的几何面积，内部重叠部分只计算一次。
- 耕地变化面积固定为“不合理流出面积－新增耕地面积”。
- 本年度耕地面积固定为“上年度耕地面积－不合理流出面积＋新增耕地面积”。
- 要素级数据问题应记录且不得中断整批分析。增量包中的空地类编码和未知编码要素仍参与增量包面积计算，但不参与后续分类指标；空几何、无法修复或修复后不是面几何的要素无法参与任何面积计算。增量包个数始终使用原始要素条数。
